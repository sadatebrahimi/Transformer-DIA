"""The command line entry point for Transformer-DIA."""
import datetime
import functools
import logging
import os
...
logger = logging.getLogger("transformer-dia")
...
@click.option(
    ...
    required=True,
    default="denovo",
    help="\b\nThe mode in which to run Transformer-DIA:\n"
...
def main(
    ...
    Ebrahimi, S. and Guo, X., 2023, December. Transformer-based de novo peptide sequencing for data-independent acquisition mass spectrometry. In 2023 IEEE 23rd International Conference on Bioinformatics and Bioengineering (BIBE) (pp. 28-35). IEEE.

    https://github.com/Biocomputing-Research-Group/Transformer-DIA
    """
    if output is None:
        output = os.path.join(
            os.getcwd(),
    ...
    logger.info("Transformer-DIA version %s", str(__version__))
    ...
    # Run Transformer-DIA in the specified mode.
    if mode == "denovo":
        logger.info("Predict peptide sequences with Transformer-DIA.")
        ...
        writer = ms_io.MztabWriter(f"{output}.csv")
        logger.info("Evaluate a trained Transformer-DIA model.")
        model_runner.evaluate(peak_path, peak_feature, model, config, writer)
        writer.save()
    elif mode == "train":
        logger.info("Train the Transformer-DIA model.")
...
def _get_model_weights() -> str:
    ...
    """
    cache_dir = appdirs.user_cache_dir("transdia", False, opinion=False)
